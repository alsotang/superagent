...
methods.forEach(function(method){
  ...
  Agent.prototype[name] = function(url, fn){
    var self = this;
    var req = request(method, url, function (_req) {
      _req.on('response', self.saveCookies.bind(self));
      _req.on('redirect', self.saveCookies.bind(self));
      _req.on('redirect', self.attachCookies.bind(self, _req));
    });
    req.ca(this._ca);
